    ...
    const StoppedDistanceAttestation& att) const
{
    return att.targetCarId + "|" + std::to_string(att.epoch) + "|" +
        toHex(att.earlyClaimHash.data(), att.earlyClaimHash.size()) + "|" + std::to_string(att.distanceToStopCm);
}
    ...
    const StoppedDistanceAttestation& att) const
{
    ...
    if (origin < 0 || att.epoch != static_cast<int>(ctx_.current_epoch_) ||
    ...
    const StoppedDistanceEcho& echo) const
{
    return std::to_string(echo.echoingReplicaId) + "|" + echo.targetCarId + "|" +
        std::to_string(echo.epoch) + "|" + toHex(echo.earlyClaimHash.data(), echo.earlyClaimHash.size()) + "|" +
        toHex(echo.attestationHash.data(), echo.attestationHash.size()) + "|" + std::to_string(echo.distanceToStopCm);
}
...
ResDBIntersectionApp::deserializeStoppedDistanceCert(BFTMessage* msg) const
{
    ...
    if (!readU32(bytes, offset, echoCount) ||
            echoCount > static_cast<uint32_t>(std::max(0, ctx_.total_vehicles_ - 1))) return {};
...
void ResDBIntersectionApp::maybeBroadcastStoppedDistanceAttestation()
{
    ...
    try {
        ...
        att.epoch = static_cast<int>(ctx_.current_epoch_);
        ...
        if (!CryptoAuth::instance().signBytes(
                ctx_.ec_private_key_, reinterpret_cast<const uint8_t*>(payload.data()),
        ...
        std::cout << "[STOPPED-DISTANCE-ATTEST] target=" << carId
                  << " epoch=" << att.epoch
                  << " earlyClaimHash=" << toHex(att.earlyClaimHash.data(), att.earlyClaimHash.size())
        ...
        if (falseDistanceTarget)
            std::cout << "[DIST-ATTACK-DECLARE] target=" << carId
                      ...
                      << " offsetM=" << 0.0 << "\n";
        const int required = (ctx_.tolerated_faults_ >= 0 ? ctx_.tolerated_faults_ :
            (ctx_.total_vehicles_ - 1) / 3) + 1;
...
void ResDBIntersectionApp::scheduleStoppedDistanceAttestationRetry()
{
    if (!stopped_distance_attestation_retry_timer_ ||
            ...
            stopped_distance_attestation_retry_count_ >=
                stopped_distance_retry_max_ ||
            ctx_.discovery_.state != DiscoveryState::COLLECTING || ctx_.propose_submitted_ ||
            ctx_.order_applied_ || crashCommsDisabled_ ||
            ctx_.current_phase_ == ConsensusPhase::DEPARTED) return;
    scheduleAt(simTime() + stopped_distance_retry_interval_sec_,
...
void ResDBIntersectionApp::retryStoppedDistanceAttestation()
{
    if (!stopped_distance_attestation_sent_ || stopped_distance_cert_broadcast_ ||
            ...
            stopped_distance_attestation_retry_count_ >=
                stopped_distance_retry_max_) return;
    ++stopped_distance_attestation_retry_count_;
    ...
    std::cout << "[STOPPED-DISTANCE-ATTEST-RETRY] target="
              ...
              << " max=" << stopped_distance_retry_max_ << "\n";
    scheduleStoppedDistanceAttestationRetry();
}
...
void ResDBIntersectionApp::handleStoppedDistanceAttestation(BFTMessage* msg)
{
    ...
    if (!bound) {
        ...
        auto cert = ctx_.collected_certs_.find(att.targetCarId);
        bound = cert != ctx_.collected_certs_.end() && cert->second.claimHash == att.earlyClaimHash;
    }
    if (!bound) {
        const std::string pendingKey = att.targetCarId + ":" + std::to_string(att.epoch)
            + ":" + toHex(att.earlyClaimHash.data(), att.earlyClaimHash.size());
        pending_distance_attestations_.emplace(pendingKey, att);
        std::cout << "[DIST-ATTEST-PENDING] witness=" << ctx_.replicaId_
    ...
    const std::string key = std::to_string(ctx_.replicaId_) + ":" + att.targetCarId + ":" +
        std::to_string(att.epoch) + ":" + toHex(attHash.data(), attHash.size());
    ...
    if (!colluding) {
        std::cout << "[DIST-PERC-EVAL] witness=" << ctx_.replicaId_
                  << " target=" << att.targetCarId << " epoch=" << att.epoch
                  << " attestationHash=" << toHex(attHash.data(), attHash.size())
        ...
        std::cout << "[DIST-COLLUSION-ECHO] target=" << att.targetCarId
                  << " epoch=" << att.epoch << " signer=" << ctx_.replicaId_
                  << " claimedDistanceCm=" << att.distanceToStopCm << "\n";
    }
    if (!accept) return;
    ...
    if (!CryptoAuth::instance().signBytes(
            ctx_.ec_private_key_, reinterpret_cast<const uint8_t*>(payload.data()), payload.size(),
...
void ResDBIntersectionApp::collectStoppedDistanceEcho(const StoppedDistanceEcho& echo)
{
    ...
    if (my_received_distance_echoes_.size() >= static_cast<size_t>(std::max(0, ctx_.total_vehicles_ - 1)))
        return;
...
bool ResDBIntersectionApp::finalizeLocalStoppedDistanceCert(const char* reason)
{
    ...
    ctx_.discovery_.localDistanceCert = LocalCertState::QUEUED;
    ...
    const simtime_t retryAt = simTime() +
        ...
        ctx_.replicaId_ * par("arrivalSlotSec").doubleValue();
...
bool ResDBIntersectionApp::validateStoppedDistanceCert(const StoppedDistanceCert& cert) const
{
    ...
    if (1 + static_cast<int>(cert.echoes.size()) < required ||
            cert.echoes.size() > static_cast<size_t>(std::max(0, ctx_.total_vehicles_ - 1))) return false;
...
void ResDBIntersectionApp::handleStoppedDistanceCert(BFTMessage* msg)
{
    ...
    const std::string& carId = cert.attestation.targetCarId;
    {
        ...
        auto arrival = ctx_.collected_certs_.find(carId);
        if (arrival == ctx_.collected_certs_.end() ||
                arrival->second.claimHash != cert.attestation.earlyClaimHash) return;
        collected_distance_certs_[carId] = cert;
    }
    std::cout << "[DIST-CERT-STORED] r" << ctx_.replicaId_ << " target=" << carId
              << " epoch=" << cert.attestation.epoch
...
// ── Timer handlers ───────────────────────────────────────────────────────────
// Registered in the dispatchTimer() table rather than an if-chain, matching how
// every other timer on this class is routed.

void ResDBIntersectionApp::onStoppedDistanceFinalize(cMessage*)
{
    finalizeLocalStoppedDistanceCert("finalize-timer");
}

void ResDBIntersectionApp::onStoppedDistanceRetry(cMessage*)
{
