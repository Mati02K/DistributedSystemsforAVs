...
```bash
benchmarks/ablations/run_ablations.sh <1..6|all> [reps]   # -> benchmarks/ablations/results/
...
Lateral observation sigma 0 to 2.0 m, all replicas honest.
...
witness misread it. The question is what that costs.

**Sigma is a physical quantity here, not a knob.** Each arm's confusion matrix
is the one implied by a Gaussian lateral error of that sigma against the
...
and checked in as `perception_matrices.csv`. The generator reproduces the
checked-in catalog byte-for-byte, so the x axis cannot drift from the geometry
it claims to describe. At sigma 0.25 m the off-diagonal mass is 7.8e-11 — below
any effect; at 2.0 m it is 21%, i.e. one observation in five lands on the wrong
approach.
...
nothing is safe and useless. Reading the three together is what shows the cost
landing where it should.

**These are N=4, 1 repetition. Not yet reportable.** The saturation at 8 s is
the certificate-collection timeout, so the right-hand half of panel 1 is
measuring the deadline, not the protocol. The frontier this study is for — the
sigma at which f+1 becomes unreachable and commits actually fail — is at larger
N, where there are more witnesses to lose. Run `run_ablations.sh 6` over the
full `NS` sweep with 3+ reps before drawing any conclusion from it.

**Not in this figure:** any measure of attack success. Every replica is honest
here. Mixing a Byzantine arm in would confound "the witness misread an honest
car" with "the witness correctly refused a liar", and separating those two is
the entire purpose of the gate. Ablation 2 covers the adversarial case.
